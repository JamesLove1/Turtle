...
int main(int argc, char* argv[]){ 
  test();
  int inputNotGraterThanOne = INPUTSEQUAL;
  if(argc > inputNotGraterThanOne){
    exit(EXIT_FAILURE);
  }
  FILE* TLLs = fopen(argv[INPUTONE],"r");
  if(TLLs == NULL){
  ...
  if(argv[INPUTTWO] != NULL){
    strcpy(script->outputFileName,argv[INPUTTWO]);
  }
  witchOutput(script, argv[INPUTTWO]);
...
void init_turtle(script* script){
  ...
  script->turtle_X = TURTELXINITALVALUE;
  script->turtle_Y = TURTELYINITALVALUE;
...
void addDataPostScript(script* PROG, double x_moveto, double y_moveto, double x_lineto, double y_lineto){
  ...
  int newIndexMax = maxIndexPS + STACKMUTIPLE;
...
void witchOutput(script* script, char *argvTwo){ 
  ...
  if(argvTwo!= NULL){
    int indexOffset = strlen(argvTwo) - INDEXOFSET;
    int numCharacters = INDEXOFSET;
    ...
    indexOffset = strlen(argvTwo) - OFSET;
    numCharacters = OFSET;
...
void ifPSsetTurtle(script* script){
  if(script->outputType == PS){
    script->turtle_X = POSTSCRIPTTURTLEX;
    script->turtle_Y = POSTSCRIPTTURTLEY;
  }
}
...
void RGT_Interp(script* file){
  ...
  if(degrees > THREESIXTYDEGREES){   
    degrees -= THREESIXTYDEGREES;
    file->turtle_Degrees = degrees;
  }  
  if(VAR(file)){
    int DollarSymbol = 0 ;
...
bool LTR(script* file, int* LTRindex){  
  *LTRindex = file->index;
  ...
  if(!strcmp(file->prevInstruction,"VAR") && lenght == INPUTTWO){
  ...
  if(prog->stack.indexStack < prog->stack.maxLen ){
    ...
    if(pushValue[0] == '$'){
      ...
      if(prog->VARArray[indexVarArray].VARType == NUMBER){
        number = prog->VARArray[indexVarArray].NUMBER;
